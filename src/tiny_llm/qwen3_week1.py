...
class Qwen3MultiHeadAttention:
    ...
    def __call__(
        ...
        x=scaled_dot_product_attention_grouped(
            ...
            k.astype(mx.float32),
            v.astype(mx.float32),
            self.scale,mask).astype(x.dtype)
...
class Qwen3MLP:
    ...
    def __call__(self, x: mx.array) -> mx.array:
        gate=mx.matmul(x,self.w_gate.T)

        up=mx.matmul(x,self.w_up.T)

        gate =silu(gate) *up

        output = mx.matmul(gate,self.w_down.T)
        return output
...
class Qwen3TransformerBlock:
    ...
    def __call__(
        ...
        attn=self.attn(input_layernorm,mask=mask)
        attn=residual+attn

        residual =attn

        post_attention_layernorm=self.post_attention_layernorm(attn)
